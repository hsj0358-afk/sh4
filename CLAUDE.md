...
이 커밋 시점에 B 실호출이 진행 중이다. 확인된 것은 **보낸 것까지**다.
...
**바뀐 것이 자료가 아니라는 증거가 첫 줄이다.** 결과(스코어 채움 수 ·
`null` 수 · 검증 오류 · 근거 유효성 · 사용량)는 실호출이 끝나는 대로
여기에 **실측 그대로** 적는다 — 14/14 를 목표로 값을 강제하지 않는다
(§18: 성공은 '전부 채웠다' 가 아니라 '자료가 충분하면 적고 모자라면
`null` 을 유지한다' 이다).

...
### 3-10. Racing Santander 의 FotMob 표기는 아직 관측하지 못했다 (5-E2)

§1-26 이 등록한 두 표기는 **후스코어드가 보낸 것**이다. FotMob 이 이 팀을
뭐라고 적는지는 보지 못했다 — `cache/probe/` 는 사용자 PC 에 있고 이 세션
에서는 fotmob.com 이 차단돼 있다(§2-1). 그래서 **추측 표기를 넣지 않았다.**

정규명(`Racing Santander`)·`Racing`·`Santander` 는 부분일치·토큰 유사도로
붙을 여지가 있지만 **그것이 붙는다는 증거는 없다.** 확인은 다음 실수집
한 줄로 끝난다.

```
[laliga] 시즌 경기 색인 N경기        722 → 760 이면 붙은 것이다
```

  · 722 그대로면 FotMob 표기가 다른 것이다. 그때 `cache/probe/` 의 라리가
    원본에서 **실제 표기를 보고** `en:` 에 한 줄 더한다 — 이름을 지어내지
    않는다(§1-22).
  · 760 이 되면 엘체·헤타페의 최근 구간이 3경기 → 4경기가 된다. 축 값이
    바뀌는 것이 정상이고, 그건 표본이 제대로 잡힌 결과다.

**강점 0개 3팀도 다음 실수집에서 갈린다.** `team_nostrength_*` 원본이
남으므로, 그 파일 안에 `(Team has no significant strengths)` 가 있으면
소스 한계가 확정이고 없으면 그때 파서를 본다.

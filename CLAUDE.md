...
프롬프트가 바뀌었으므로 `MODERATOR_PROMPT_VERSION` 을 `"1"` → `"2"` 로
올렸다 — 옛 캐시는 미적중이다. **채팅 경로의 지침 지문도 바뀐다**
(`a394547f` → `dff0793c`). 프로젝트 지침을 다시 붙여넣어야 한다 (§1-11-1).
...
단계마다 회차 전 경기가 한 파일에 들어 있다.

```
00_프로젝트_지침.md            채팅 프로젝트 지침에 넣는다
01_1단계_데이터분석가.md        회차 전 경기 자료 (역할 A)
02_2단계_맞대결분석가.md        **같은** 자료 (역할 B)
03_3단계_사회자.md             1·2단계 응답을 [A]·[B] 에 붙여넣는다
...
불변조건은 그대로 지킨다.
  ...
  · 두 분석가는 **같은 자료**를 받는다 — 1·2단계 파일의 payload 블록이
    문자열까지 같은지 테스트가 본다.
  · 사회자는 **축 지표 덤프를 다시 받지 않는다** (§1-10). `build_input()`
    이 줄인 그대로 낸다 — 실측에서 회차 전체 사회자 파일이 **10.7KB** 다
    (분석가 파일 347KB 대비 3%).
  · 사회자는 **경기마다 종합 예상 스코어 하나를 채택한다** (§1-10).
    3단계 시트의 채팅 메시지가 `adopted_home`·`adopted_away`·
    `score_rationale` 을 명시적으로 요구한다 — 실행에서 사회자가 A·B 를
    비교만 하고 끝나는 일이 있었다. 규칙 본문은 여기 베끼지 않고
    `moderator.SYSTEM` 에서 그대로 온다.
...
역할 지정("역할 A 로만 수행, 다른 역할은 하지 마십시오"), 태그 설명, 배열
출력 요구가 들어 있다. 3단계는 그 안의 `◀ … ▶` 두 자리에 1·2단계 응답
배열을 채운다.

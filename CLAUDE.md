...
**기간마다 표를 따로 낸다.** 갈래 셋(결과 2-A · 공격 2-B · 수비 2-C) ×
기간 둘(시즌 · 최근 N) 로 최대 여섯 표이고, 각 표는 `지표 | 홈 | 원정` 이다.
...
그려진다(510,685 → 555,858 bytes, 블록 14개, 외부 참조 0, SVG 110개 정상,
빈 칸 0). 이건 의도한 변화다. 3-D 의 '패널 없이 돌린 리포트는 바이트까지
같아야 한다' 는 **패널 CSS 에 대한 규칙**이지 리포트에 블록을 더하지 말라는
뜻이 아니다.

**정성 블록(`_traits_block`)을 이 블록 바로 뒤로 옮겼다.** 예전에는 카드의

...
두 의견을 **비교·종합**하고, 이 경기의 **종합 예상 스코어 하나를 채택**한다.
세 번째 분석가가 아니다.
...
**스코어는 '채택' 이지 '평균' 이 아니다.** 처음에는 `ModeratorResult` 에
스코어 칸을 **아예 두지 않았다** — 자리가 없으면 `2-1` 과 `1-1` 을 `1.5-1`
로 만들 수 없다는 생각이었다. 그런데 실제로 돌려 보니 사회자가 A·B 를
견주기만 하고 끝나 **"그래서 이 경기는 몇 대 몇인가" 에 닿지 못했다**
(260050 3단계 실행: 14경기 전부 `score_comparison` 문장까지).

지금은 칸을 두되 **제약을 프롬프트가 아니라 구조가 건다.** §1-12 가 나눈
"구조가 막는 것" 쪽으로 옮긴 것이다.

| | 예전 | 지금 |
|---|---|---|
| 평균을 막는 방법 | 칸이 없다 | **값의 출처가 제안 집합으로 제한된다** |
| 사용자가 얻는 것 | 비교 문장 | 비교 문장 **+ 채택한 스코어와 그 이유** |

  · 채택할 수 있는 값은 **의견이 실제로 낸 `(홈, 원정)` 조합뿐**이다.
    `parse_result(allowed_scores=…)` 가 그 집합에 없으면 거부한다 —
    `1.5` 는 정수가 아니라 막히고, `2-1`·`1-1` 사이의 `2-2` 같은 합성값도
    제안에 없으므로 들어오지 못한다. 실측: 제안 `3-1`·`2-1` 에 `2-2` 를
    보내면 `채택한 스코어 2-2 를 낸 의견이 없습니다 (제안: 3-1, 2-1)`.
  · `adopted_from` 이 가리키는 역할의 스코어와 **실제로 일치해야** 한다.
    "B 의 스코어를 채택했다" 면서 A 의 숫자를 적을 수 없다. 빼먹으면
    제안 집합에서 채운다 — 새로 만드는 것이 아니다.
...
프롬프트가 바뀌었으므로 `MODERATOR_PROMPT_VERSION` 을 `"1"` → `"3"` 으로
...
회귀 테스트: `python tests/test_moderator.py` (67개).
...
**사회자 종합의 맨 앞이 채택한 종합 예상 스코어다** (`_adopted_block`,
§1-10). 사용자가 3단계에서 얻으려는 답이라 비교 문장보다 먼저 놓는다.
어느 의견에서 온 값인지와 채택 이유를 함께 적고, **평균이 아니라는 것을
...
| 종합 스코어 못 고름 | "두 의견 중 하나를 고를 근거가 자료에 없었습니다" + 사유 |
...
회귀 테스트: `python tests/test_panel_render.py` (44개).
...
나머지 불변조건은 그대로다.
  ...
  · 사회자는 **축 지표 덤프를 다시 받지 않는다** (§1-10). `build_input()`
    ...
    (자료 파일 343KB 대비 2%).
  · 사회자는 **경기마다 종합 예상 스코어 하나를 채택한다** (§1-10).
    3단계 메시지가 `adopted_home`·`adopted_away`·`conclusion` 을 명시적으로
    요구한다 — 실행에서 사회자가 A·B 를 비교만 하고 끝나는 일이 있었다.
    규칙 본문은 여기 베끼지 않고 `moderator.SYSTEM` 에서 그대로 온다.

**지침은 한 번만 넣는다 — 지문으로 낡음을 알린다.** 지침은 매 회차 같은
내용이라 프로젝트에 한 번만 붙여넣으면 되는데, 이 글은 코드의 프롬프트에서
만들어지므로 **프롬프트가 바뀌면 붙여넣은 사본이 조용히 낡는다.**
`instructions_fingerprint()`(8자, 지금 `2865633d`)가 지침 맨 위와 실행 로그에 함께 찍히므로,
...
회귀 테스트: `python tests/test_panel_export.py` (51개).
...
python tests/test_moderator.py             # 사회자 3-C (67개)
python tests/test_panel_render.py          # 패널 리포트 출력 3-D (44개)
python tests/test_panel_export.py          # 채팅용 자료 내보내기 §1-11-1 (51개)

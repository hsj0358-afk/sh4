...
### 1-46. 공통 compact packet (Phase 6-F-10) — `toto/panelpacket.py`
...
PanelPayload 14개 → build_panel_index() → PanelIndex
                  → build_compact_packet() → **packet 하나**
                  → packet_text() → A·B 의 stdin 에 **같은 문자열**
```

> **초판은 역할별로 나눴다가 되돌렸다.** `build_analyst_packet(role)` 와
> `ROLE_VIEWS` 가 `qualitative` 를 B 에게만 보냈는데, **자료를 역할로
> 가르는 그 구조 자체가 §1-9 불변조건 2 가 금지하는 것**이다. 아래
> '역할 차이는 프롬프트에만 있다' 절이 그 교정이고, 그 밖의 설계(legend
> 접기·무손실·감사·캐시)는 초판 그대로다.
...
| **공통 packet** (A·B 가 같은 것을 받는다) | **612,886** | **336,935** (예상) | **−64.8%** |

`sha256 8e919c77693c5423…` — A 와 B 의 stdin 이 이 한 값이다.

초판의 역할별 크기는 A 597,146자 · B 612,871자였다. **공통 packet 은 B 판과
같은 칸을 담으므로 B 쪽 크기에 맞춰졌고**, 늘어난 15자는 `packet_version`
문자열이 길어진 것이다. A 기준으로는 +15,740자(+2.6%)이고, 그 대가로
**불변조건이 다시 바이트 동일성으로 검사된다.**

**A 와 B 를 둘 다 실제로 불렀다** (실물 260052 · sonnet · 같은 문자열).
6-F-10 초판이 "B 는 실호출로 재지 않았다" 고 남겨 둔 자리를 메운다.

| | turns | 시간 | 실제 입력 토큰 | 출력 | 비용 | 검증 |
|---|---:|---:|---:|---:|---:|---|
| 초판 A (역할별 597,146자) | 1 | 172초 | 328,326 | 18,114 | $1.4815 | 14경기 · 오류 0 |
| **공통 A** | **1** | 106초 | **335,526** | 10,372 | $1.4329 | **14경기 · 오류 0** |
| **공통 B** | **1** | 157초 | **336,583** | 18,224 | $1.5156 | **14경기 · 오류 0** |

**두 실행의 `stdin.txt` 가 바이트까지 같다** — 디스크에서 잰 sha256 이
둘 다 `8e919c77693c5423…` 이고, `system.md` 는 `a073c815…` ↔ `bc876998…`
로 다르다. 설계가 그렇다는 것이 아니라 **실제로 보낸 파일이 그렇다.**

  · 근거 ID 는 두 역할 · 28건 전부 **그 경기의 것**이었다.
  · **A 의 응답에 정성 자료가 한 번도 나오지 않는다**(0/14). packet 에는
    있는데 역할 프롬프트가 언급하지 않아서다. **B 는 12/14 에서 쓴다** —
    1번 경기에서 `노팅엄F 공중볼 강점(Strong) ↔ A빌라 공중볼 약점(Very
    Weak)` 을 짚었다. 같은 자료를 주고 프롬프트로 갈랐다는 것이 실물에서
    이렇게 보인다.
  · 입력 토큰이 1,057 차이 나는 것은 packet 이 아니라 **시스템 프롬프트
    길이**(A 2,699자 · B 5,063자) 때문이다.

**B 가 14경기 전부 스코어를 내지 않았다(`null`).** 계약상 허용된 값이고
(§1-5 — 스코어를 안 낸 것과 0-0 을 예상한 것은 다르다) 검증도 통과하지만,
그대로 3단계에 가면 **사회자의 제안 집합이 A 것뿐**이 된다. 원인은 이
Phase 가 아니다 — B 가 받은 자료는 초판 B packet 과 사실상 같고
(`packet_version` 15자 차이), 프롬프트는 한 글자도 바뀌지 않았다
(PANEL 4). 이번에 **처음 실호출로 봤을 뿐**이다. 프롬프트 소관이라
여기서 고치지 않았다.
...
공통 packet 실호출에서 다시 재면 **1.827**(612,886자 → 335,526토큰)인데
**값을 바꾸지 않았다** — 1.819 로 잡으면 336,935 로 0.42% 크게 나오고,
문맥 여유는 넉넉히 잡는 쪽이 틀렸을 때 안전하다.

#### 새 통계를 만들지 않는다

**고르고 다시 배열할 뿐이다.** `_pack_axis`·`_pack_side`·`_pack_quality`·
`build_compact_packet`·`build_panel_index` 에 **산술이 한 줄도 없고**
...
자료를 역할에 따라 고르는 분기가 이 모듈에 없다 — `role` 인자를 받는
함수도, 역할을 키로 하는 view 표도 없다. `ROLE_VIEWS` ·
`build_analyst_a_packet` · `build_analyst_b_packet` 이라는 이름이 코드에
남아 있지 않은 것까지 테스트가 본다.

```
A 의 stdin  ==  B 의 stdin        (바이트 · sha256 동일)
A 의 system  ≠  B 의 system       (역할 프롬프트는 그대로다)
```

**칸은 합집합이다** (`PACKET_FIELDS`). 초판에서 B 에게만 가던
`qualitative`(정성 특성·관계)가 여기 들어간다. 데이터 분석가의 역할
프롬프트는 그 칸을 **한 번도 언급하지 않으므로**("관측된 **수치**만")
무엇을 볼지는 프롬프트가 정하고, 자료는 같게 둔다.

  · 크기가 이유였던 적이 없다 — `qualitative` 의 기여는 **0.9%** 다.
  · `conflicts` 가 비면 싣지 않는 규칙은 그대로인데, **packet 이 하나라
    유무가 두 역할에 자동으로 같다.**
  · packet 에 **역할 지시가 없다.** `"당신은 …분석가"`·`data_analyst`·
    `matchup_tactical_analyst` 가 본문에 한 번도 나오지 않는다 — 자료와
    지시를 한 문서에서 읽히게 하지 않는다 (§1-11-1 과 같은 이유).

**한 번 만들어 둘이 나눠 쓴다.** 두 번 만들면 같은 자료에서도 갈릴 여지가
생기므로(직렬화·legend 번호가 순서에 달려 있다), `preflight()` 가
`Preflight.packet_text` 에 담고 `_run_stages` 가 그 문자열을 두 호출에
넘긴다. 실제로 **호출 3회에 packet 생성이 1회**인 것을 테스트가 센다.

**§1-9 불변조건 2 를 원래 형태로 되돌린 것이다.** 초판은 그 단언을 "정량
본체가 바이트까지 같다" 로 좁혔었는데(그 조항의 실질이 비교 가능성이라는
이유로), 조항의 문면 — **"두 분석가는 같은 `PanelPayload` 객체와 같은
직렬화 문자열을 받는다"** — 이 더 검사하기 쉽고 새는 자리가 없다.
`test_l3`·`test_ab_identical_packet`·`test_ab_identical_hash` 가 고정한다.
...
`index.json` · `common_packet.json` · `stats.json` 을 남긴다. **LLM 산출물을
담지 않는다**(테스트). `.gitignore` 에 넣었다 — 원본에서 다시 만들 수 있는
파생물이라 `panel_work/` 와 같은 취급이다.

  · **packet 파일은 하나다.** 두 벌을 남기면 나중에 둘을 견주다가 같은지
    다른지를 파일에서 판정하게 되는데, 같아야 하는 것을 두 번 적어 두는
    것 자체가 갈릴 자리를 만드는 일이다.
  · **보낸 문자열 그대로** 남긴다 — 다시 직렬화하면 캐시의 sha 가 실제로
    보낸 것과 갈릴 여지가 생긴다.
  · `PACKET_VERSION` 이 `"1"` → **`"6-F-10-common-v2"`** 로 바뀌었다.
    같은 원본에서 다른 packet 이 나오므로 옛 캐시는 미적중이다 (§1-4).
    초판이 남긴 `analyst_a_packet.json`·`analyst_b_packet.json` 은
    `LEGACY_PACKET_FILES` 에 이름만 남아 있고 **쓰는 곳이 없다**.

#### 데모에서는 줄지 않는다 — 그게 정상이다

데모 픽스처는 분석 축이 없어 packet 이 원본보다 **커질 수도** 있다 —
실측 3경기 **−4.9%**(늘어난다) · 14경기 **+2.4%**. legend 에는 고정 비용이
있고 데모에는 접을 반복이 없기 때문이다. **감소율은 자료에 달렸고 목표치를
코드에 박지 않았다** (§4) — `150000` 같은 수가 모듈에 없는 것을 테스트가
본다.

같은 이유로 **줄었다는 것 자체를 기준으로 삼지 않는다.** 이번 교정은
packet 을 2.6% 키우는데, 그것이 옳은 방향이다 — 기준은 최소 토큰 수가
아니라 A·B 동일성이다.
...
**아직 확인하지 못한 것.**
  ...
  · **A·B·C 를 이어서 돌린 적은 없다.** 위 둘은 단계 하나씩 부른 것이고,
    `run()` 전체 흐름(조립 → C → [4])은 mock 으로만 확인했다. B 가
    스코어를 내지 않는 위 상태에서 C 가 무엇을 하는지는 실물로 보지
    못했다.

회귀 테스트: `python tests/test_panel_auto.py` (123개 — M절이 6-F-10).
A·B 동일성은 `test_l3` · `test_ab_identical_packet` ·
`test_ab_identical_hash` · `test_common_packet_single_generation` ·
`test_role_data_not_embedded` · `test_common_packet_contains_previous_union` ·
`test_a_b_prompts_differ` 가 고정한다. 변경 전 트리에 새 테스트를 돌리면
**21개가 깨지고**, 그중 `test_l3` 가 `A·B 의 stdin 이 다르다` 로 깨진다 —
이번 교정이 실제로 바꾼 것이 그것이다(음성 대조).

### 1-47. B 가 예상 스코어를 내지 않았다 (Phase 6-F-11) — 프롬프트의 비대칭

6-F-10 실호출에서 맞대결·전술 분석가가 14경기 **전부** `predicted_home`·
`predicted_away` 를 `null` 로 냈다. 스키마 위반이 아니고(계약상 `null` 은
허용된다) 분석 내용도 정상이었다 — 근거 20~30개를 인용했고 정성 관계도
12/14 에서 실제로 썼다. 그런데 그대로 3단계에 가면 **사회자의 제안 집합이
A 것뿐**이 되어 스코어 비교가 비대칭이 된다.

#### 원인은 자료가 아니라 프롬프트였다

두 역할 프롬프트를 나란히 놓으면 바로 보인다.

```
데이터 분석가   "당신의 예상 스코어는 지표와 근거에서 나와야 하며 시장에
                맞추거나 일부러 어긋나게 하지 마십시오."      ← 있다
맞대결 분석가   (예상 스코어를 언급하는 줄이 **한 줄도 없다**)  ← 없었다
```

공통 규칙(`SYSTEM_COMMON`)은 반대 방향만 말한다 — **"예상 스코어에서
승/무/패를 도출하지 마십시오"** 와 **"자료가 부족하면 부족하다고
명시하십시오"**. 그래서 B 에게는 그 칸을 **선택 사항으로 읽고 기본값을
`null` 로 잡을 이유만** 있었다. 자료는 A 와 **글자까지 같았으므로**
(6-F-10 이 sha256 으로 고정했다) 자료 쪽을 의심할 여지가 없다.

#### 고친 것은 B 역할 프롬프트 **하나**다

`ROLE_PROMPTS[MATCHUP_ANALYST]` 끝에 문단 하나를 더했다. 기존 문장은 한
글자도 지우지 않았다(테스트로 고정) — 6-E-4·6-E-6 이 넣은 `qualitative`·
`style_of_play`·정량/정성 분리가 그대로 있다.

  · **스코어는 승무패가 아니다.** "그 두 칸은 자료에서 읽어낸 **예상
    득점수**이지 승/무/패 선택이 아닙니다" 를 명시했다 — 스코어를 적는 것과
    승무패를 고르지 않는 것이 어긋나지 않는다는 것을 모델이 알아야 한다.
  · **승무패에서 역산하지 않는다.** `"홈이 이길 것 같으니 2-1"` ·
    `"무승부 성격이니 1-1"` 을 예로 들어 막았다 — 그건 승무패 판단을
    스코어로 포장한 것이다.
  · **새 수치를 만들지 않는다.** 기대득점 공식·공격력 점수·가중평균·
    보정지수·확률 어느 것도 만들지 않는다 (§1-9 불변조건과 같은 금지).
  · 적었으면 `rationale` 하나가 그 스코어로 이어지는 자료 관계를 설명하고
    **기존 Evidence ID** 를 `evidence_ids` 에 넣는다. 새 ID 는 만들지 않는다.

**`null` 을 금지하지 않았다.** 이 Phase 의 목표는 **불필요한 `null` 제거**
이지 `null` 제거가 아니다 — 자료가 실제로 모자랄 때도 값을 지어내게 만들면
§1-5 와 정면으로 어긋난다. 그래서 문구는 조건부다.

```
자료가 실제로 모자라 득점 수준을 근거 있게 말할 수 없을 때만 `null` 로
두십시오. 다만 **자료가 적어 보인다는 인상만으로 `null` 을 고르지
마십시오** — 먼저 말할 수 있는지 보고, 그다음에 정하십시오.
```

`무조건`·`반드시 모든`·`null 금지`·`14경기 전부` 같은 강제 문구가 들어가지
않은 것을 테스트가 본다. **검증기도 조이지 않았다** — `panel._score()` 는
한 줄도 바뀌지 않았고 정수·`null` 두 모양이 그대로 통과한다.

#### 자료·구조는 한 칸도 바뀌지 않았다

`panelpacket.py`·`panelauto.py`·`panelwork.py`·`panelimport.py`·
`panelpaste.py`·`panelaudit.py`·`panelexport.py`·`moderator.py`·`render.py`·
`analyze.py`·`models.py`·`predict.py`·`briefing/` **diff 0줄**이고, A 역할
프롬프트(306자)도 그대로다. 공통 packet 의 sha256 이 6-F-10 과 **같다**
(`8e919c77693c5423…`) — 바뀐 것이 자료가 아니라는 직접 증거다.

  · `--demo` **675,280** · `--rerender-artifact 260052` **949,635** ·
    경기자료 MD **1,209,043** 그대로.
  · `MODERATOR_PROMPT_VERSION` **6** · Panel Result schema **1.1** 그대로.

**`PANEL_PROMPT_VERSION` 4 → 5 · 지침 지문 `fe098456` → `4a54e7ad`.**
프롬프트를 고쳤으므로 패널 캐시는 미적중이고, `ROLE_PROMPTS` 가 채팅 지침
본문에 그대로 실리므로 **프로젝트 지침을 다시 붙여넣어야 한다** (§1-11-1).

#### 실호출 — 같은 자료, 다른 프롬프트

**보낸 것부터.** 바뀐 것이 자료가 아니라는 증거가 첫 줄이다.

| | 6-F-10 | 6-F-11 |
|---|---|---|
| stdin sha256 | `8e919c77693c5423…` | **같다** |
| B `system.md` | 5,063자 | **6,776자** (+1,713 = 새 문단) |
| A `system.md` | 1,309자 | 그대로 |

**결과 (실물 260052 · sonnet · 같은 packet).**

| | 6-F-10 B | 6-F-11 B |
|---|---:|---:|
| turns | 1 | **1** |
| 시간 | 157초 | 272초 |
| 실제 입력 토큰 | 336,583 | **337,324** |
| 출력 토큰 | 18,224 | 26,273 (사고 19,726) |
| 비용 | $1.5156 | $1.5991 |
| 검증 오류 | 0 | **0** |
| 근거 ID 오류 | 0 | **0** |
| **스코어를 낸 경기** | **0/14** | **14/14** |

```
[B score output]
matches: 14
home score filled: 14/14
away score filled: 14/14
fully scored: 14/14
null: 0/14
errors: 0
invalid evidence: 0
...
**14/14 는 목표가 아니라 관측값이다** (§18). 성공 기준은 '전부 채웠다' 가
아니라 '자료가 충분하면 적고 모자라면 `null` 을 유지한다' 이므로, 이 회차에
`null` 이 하나도 남지 않은 것은 **그 14경기의 자료가 충분했다**는 뜻이지
프롬프트가 값을 강요했다는 뜻이 아니다 — 코드에 `14` 를 성공 조건으로
박지 않았고, `null` 은 계약·검증기·프롬프트 세 곳 모두에서 그대로 열려 있다.

**품질도 함께 봤다** (§19). 자동 검사에서 전부 0이다.

  · 승무패 문구 — `추천`·`픽`·`베팅`·`홈승`·`원정승`·`승리할 것` **0회**
  · 새 통계 — `가중`·`종합점수`·`공격력 점수`·`보정`·`확률`·`지수` **0회**
  · 정성 자료 활용 **14/14** (6-F-10 은 12/14)
  · 스코어 ↔ rationale 연결이 실제로 있다. 1번 경기(`0-1`)의 근거 넷째가
    `시즌 실제 득점 수준(A빌라 0, 노팅엄F 0.67)이 예상 스코어의 직접적
    근거이며, 이는 E005·E014 의 저조한 득점 전환과 일치한다` 이고,
    다섯째는 `style_of_play 칸은 이 경기 payload 에 없음` 이다 — 없는 것을
    없다고 적는 규칙도 함께 지켜졌다.
  · 스코어 분포가 `0-1`·`2-0`·`0-0`·`1-4`·`2-2` 로 갈린다. 한 값으로
    몰리지 않았다는 뜻이고, 승무패로 보면 홈 2 · 무 3 · 원정 9 다 —
    **그 낱말은 프로그램도 B 도 적지 않는다** (§1-3).

#### 3세션 E2E — A · B · C

B 가 정상화된 뒤 C 를 불렀다 (§21). C 입력은 **사회자 자료 + A 결과 +
B 결과 뿐**이고 회차 원본이 다시 가지 않는다 — 실측 `119,871자` 에
`<panel_payload` 가 **0회**다 (§1-45 의 축소 규칙 그대로).

조립은 **기존 CLI 경로**를 그대로 탔다. 새 경로를 만들지 않았다.

```
--save-panel-opinion --role a   ok (14/14경기 · 데이터 분석가)
--save-panel-opinion --role b   ok (14/14경기 · 맞대결·전술 분석가)
--build-moderator-input         ok (14경기 → 03_사회자자료_완성.md)
--save-moderator-result         ok (14/14경기) ← C 결과
--paste-panel-result            PASS · 커버리지 COMPLETE · 리포트 976.4 KB
```
...
| turns | 1 | 1 | 1 |
| 시간 | 106초 | 272초 | 180초 |
| 실제 입력 토큰 | 335,526 | 337,324 | **77,755** |
| 출력 토큰 | 10,372 | 26,273 | 18,021 |
| 비용 | $1.4329 | $1.5991 | **$0.4783** |

C 가 싼 이유가 §1-45 의 축소 규칙이다 — 회차 원본도 packet 도 가지 않고
사회자 자료와 A·B 결과만 간다(입력이 B 의 **23%**).

**이 Phase 가 실제로 푼 것이 여기서 보인다.**

```
            6-F-10          6-F-11
A 원안      14경기          14경기
B 원안      **없음**        14경기
견줄 수 있나  아니다          **원안 일치 6 · 불일치 8**
```

불일치 8경기에서 사회자가 **B 원안을 7번 채택**했다(`adopted_from`
= `matchup_tactical_analyst`). 3번 경기는 분포에 `compromise` 가 함께
나타났다 — A `2-2` · B `1-1` 에서 `1-1` 을 채택했다. 6-F-10 이라면 이
여덟 경기 전부 비교할 상대가 없었다.

  · `adopted_from` 은 모델 말을 믿지 않고 **제안 집합에서 다시 정한
    값**이다 (§1-10). 그것이 B 를 가리킨다는 것은 C 가 B 의 스코어를
    실제로 봤다는 뜻이다.
...
#### 기존 핀 다섯의 범위를 옮겼다 — 기대값을 바꾼 것이 아니다
  ...
  · `test_panel_auto.test_e7`·`test_k5` · `test_panel_work.test_p1` —
    6-F-3·6-F-6·6-F-7 이 "이 Phase 는 프롬프트를 건드리지 않았다" 를
    **숫자로** 적어 둔 범위 선언이고, 6-F-11 이 바로 그것을 하는 Phase 다
    (§1-29·§1-31·§1-42 와 같은 교정). 지키려던 것 — 사회자·스키마는 그대로 ·
    자리를 옮겨도 시스템 글자가 같다 · `panelwork` 는 프롬프트를 만지지
    않는다 — 은 새 자리에서 그대로 확인한다.
  · `test_panel.test_25g` — `panel.py` 소스에 `평균` 이라는 낱말이 없는지
    보는 검사인데, 새 프롬프트의 **금지 문구**(`가중평균 … 만들지 않습니다`)
    에 걸렸다. §1-38 의 `test_25d`(`strength` ↔ `strength_side`)와 같은
    종류다 — 이 테스트가 지키는 것은 **코드**이므로 프롬프트 상수를 걷어낸
    뒤 코드만 보고, 프롬프트 쪽에서는 그 낱말이 **금지로만** 쓰이는지 따로
    확인한다(더 강해졌다).
  · `test_panel_export.test_t10` · `test_relationship_delivery.test_g5` —
    지문 핀이다. **바뀌는 것이 그 장치의 목적**이라 새 값으로 옮겼다.

회귀 테스트: `python tests/test_b_score_prompt.py` (19개). 변경 전 트리에
돌리면 **9개가 깨지고**, 그중 `test_b_prompt_mentions_score_output` 이
`B 프롬프트가 predicted_home 를 말하지 않는다` 로 깨진다 — 이번 Phase 가

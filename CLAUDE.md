...
**남은 것 하나 — 레이더 라벨이 7.7px 잘린다.** Chromium 실측에서
`상대 박스 터치` 가 viewBox 왼쪽으로 넘친다(14개 카드 전부). **첨부된
260052 원본에서도 같은 값(-7.7)이 나온다** — 4-E 의 `VB_PAD` 가 이 글꼴에서
모자란 것이고 5-D 가 만든 것이 아니다. 레이더 geometry 를 건드리지 않기로
한 Phase 라 손대지 않았다. 고칠지는 사용자가 정한다.

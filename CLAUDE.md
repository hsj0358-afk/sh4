...
배치는 `지표: 홈 시즌 · 홈 최근N · 원정 시즌 · 원정 최근N` 이다. **기간을
열로 두면 시즌 값과 최근 값이 구조적으로 섞일 수 없다** (§1-1-7 이 키에
기간을 넣은 것과 같은 이유). 갈래는 셋 — 결과(2-A) · 공격(2-B) · 수비(2-C).

- **값이 하나도 없는 기간 열은 내지 않는다.** `—` 만 채운 열은 자리만
  차지하면서 '재 봤는데 없다' 처럼 보인다. 기간이 하나만 남으면 머리글도
  한 줄로 접는다. 데모(시즌만 있음)에서 실제로 이 경로를 탄다.
...
그려진다(510,685 → 544,806 bytes, 블록 14개, 외부 참조 0, SVG 110개 정상).
이건 의도한 변화다. 3-D 의 '패널 없이 돌린 리포트는 바이트까지 같아야 한다'
는 **패널 CSS 에 대한 규칙**이지 리포트에 블록을 더하지 말라는 뜻이 아니다.

회귀 테스트: `python tests/test_axes_render.py` (24개).
...
python tests/test_axes_render.py           # 경기력 분석 블록 §1-1-15 (24개)

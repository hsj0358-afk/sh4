> **이 저장소에는 두 개의 도구가 있습니다.**
> - 📰 **아침 신문 브리핑 자동화** (`briefing/`) — 아래 문서
> - ⚽ **축구토토 승무패 분석 리포트** (`toto/`) — [`docs/toto_usage.md`](docs/toto_usage.md)
>   베트맨 승무패 14경기를 수집해 배당 내재확률·리그 백분위 레이더·최근폼·상대전적·
>   전략적 상성을 담은 HTML 리포트를 만듭니다. `python -m toto --demo` 로 바로 확인 가능.

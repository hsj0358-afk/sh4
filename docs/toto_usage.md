...
```
  ...
  [5] 캐시 비우고 새로 수집
  [6] 후스코어드 수집 실패 진단
  [7] 데이터 소스 점검 (접속해서 구조 확인)
  [8] 저장된 점검 응답 다시 분석 (접속 안 함)
  [0] 종료
```
...
| `--skip-match-details` | 경기 상세 생략. FotMob 순위·폼은 그대로 받고 **최근 경기 슈팅·xG 프로필**만 빠진다. 경기당 요청 1회를 아낀다 |
...
| 최근 경기 슈팅·xG 프로필 | **npxG(PK 제외)·xGOT·오픈플레이/세트피스 xG·슈팅·피슈팅·유효슈팅·박스 안 슈팅 비율.** 시즌 누계가 아니라 **최근 N경기 평균**이라 블록을 따로 두고 표본 크기를 함께 적는다 | 다이버징 바 |
...
| 순위표(홈/원정)·시즌 xG·폼·맞대결 | FotMob 리그 응답 | Playwright (리그당 1회) |
| 시즌 팀 통계 23종 | FotMob `stats.teams[]` 피드 | Playwright (지표당 1회, 그 리그 전 팀) |
| npxG·xGOT·슈팅/피슈팅·박스 안팎 | FotMob **경기 상세** (`matchDetails`) | Playwright (최근 경기당 1회) |
...
리포트는 외부 참조가 하나도 없는 **자체 완결 HTML 한 개**다. 파일 하나만
폰으로 넘어가면 그대로 열리고, 표는 가로 스크롤이 걸려 있으며 픽을 눌러
바꾸는 단통표도 순수 JS 라 폰에서 동작한다.

실행이 끝나면 **클라우드 동기화 폴더에 사본을 자동으로 넣는다.**
OneDrive · 구글드라이브 · Dropbox 가 설치돼 있으면 알아서 찾아
`축구토토` 폴더를 만들고 두 개를 넣는다.

| 파일 | 용도 |
|---|---|
| `toto_260044.html` | 회차별 보관용 |
| `최신리포트.html` | **항상 최신**. 폰에서 즐겨찾기를 여기 한 번만 걸어두면 된다 |

찾는 폴더: OneDrive · 구글드라이브(가상 드라이브 `G:\내 드라이브` 포함) ·
Dropbox · 네이버 MYBOX · iCloud · Nextcloud · MEGA · pCloud.

**회사 계정 OneDrive 는 기본으로 제외**한다. 개인 파일을 회사 폴더에 쌓지
않기 위해서다. 개인 OneDrive 를 쓰려면 `copy_to_exclude` 를 비운다.

```yaml
output:
  copy_to: ["D:/내문서/축구토토"]   # 직접 지정 (이걸 쓰면 자동 탐색 안 함)
  copy_to_exclude: ["OneDrive"]      # 자동 탐색에서 뺄 폴더
```

### 클라우드를 아예 안 쓰고 싶다면 — 메뉴 [9]

같은 와이파이에 있는 폰에서 바로 여는 방법이다. 계정도, 설치도 필요 없다.

```
  [9] 폰에서 열기 (같은 와이파이)
```

주소가 뜨면 폰 브라우저에 한 번만 입력하고 즐겨찾기 해두면 된다.

```
    http://192.168.0.12:8899/
```

처음 실행하면 윈도우 방화벽이 묻는데 **'개인 네트워크'** 를 허용한다.
PC 가 켜져 있고 폰이 같은 와이파이일 때만 열린다. 끄려면 Ctrl+C.

수집과 동시에 하려면 `python -m toto --round 260044 --serve` 를 쓴다.

동기화 폴더가 없으면 아무 일도 하지 않고, 복사가 실패해도 리포트 생성 자체는
성공으로 끝난다.

> 모바일에서 **수집을 직접 돌리는 것**은 안 된다. 베트맨·FotMob·후스코어드가
> 전부 실제 브라우저(Playwright + Chromium)를 요구하는데, iOS 는 브라우저
> 자동화가 불가능하고 안드로이드는 Playwright 용 Chromium 빌드가 없다.
> 수집은 PC 에서 하고 결과만 폰에서 본다.

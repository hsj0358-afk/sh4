...
# 6: 정성 관계를 **분석가가 적은 만큼만** 다룬다 (6-E-6). 사회자 입력에는
#    `qualitative` 원문이 없는데(§1-10 의 축소는 그대로 둔다) 두 의견이
#    그것을 언급하므로, 뜻을 적어 두어 '강점 충돌 = 우위'·'공통 취약 =
#    공략' 으로 재해석하는 것을 막는다.
MODERATOR_PROMPT_VERSION = "6"
...
- 두 의견이 팀의 **강점·약점과 그 관계**(정성 특성)를 언급할 수 있습니다.
  그러나 **당신은 그 원문을 받지 않았습니다** — 당신 자료에 그 칸이 없습니다.
  그러므로 **분석가가 실제로 적은 내용만** 견주고, 거기 없는 관계를 새로
  만들거나 넓히지 마십시오. 언급된 관계의 뜻은 이렇습니다.
    · **강점 충돌** — 양 팀 모두 그 영역이 강점이라는 뜻입니다. 어느 팀의
      우위가 아닙니다.
    · **공통 취약 영역** — 양 팀 모두 그 영역이 약점이라는 뜻입니다. 어느
      팀이 상대의 약점을 공략한다는 뜻이 아닙니다.
    · **상대 약점 공략** — 이것만 방향이 있습니다. 한쪽의 강점이 상대의
      약점과 맞물린다는 뜻입니다.
  관계의 **개수**를 세어 어느 쪽이 강하다고 쓰지 마십시오.

...
def league_cache_key(league_key: str, season: str | None = None) -> str:
    ...
    **대회가 곧 캐시 정체다.** `ucl` 과 `uel` 과 `epl` 은 서로 다른 키라
    ...
    지금은 아무 호출부도 시즌을 넘기지 않는다. 6-D-7 당시에는 과거 시즌
    요청이 production 경로에서 되는지 확인되지 않아서였는데, **그 질문은
    뒤에 답이 났다** — 실측으로 시즌 질의 하나면 통한다 (§1-33). 남은 것은
    "무엇을 언제 받을지" 이고 그것이 6-D-6 소관이다. 그래서 자리는 그대로
    둔다 — **여기 한 곳만** 넘기면 `ucl 2025/2026` 과 `ucl 2026/2027` 이
    자동으로 다른 칸에 앉는다.
    """
    if not season:
